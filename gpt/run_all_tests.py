#!/usr/bin/env python3
"""Run all GPT-5.6 (OpenAI) Responses API feature tests sequentially."""
import subprocess
import sys
import os
import time

from helpers import MODEL_ID, REGION

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
tests = sorted(f for f in os.listdir(SCRIPT_DIR) if f.startswith("test_") and f.endswith(".py"))

print(f"Running {len(tests)} feature tests against Bedrock Responses API (bedrock-mantle)...")
