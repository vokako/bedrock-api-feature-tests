...
| Web Search Tool | ✅ | ❌ | ⚠️ | tool type 认可但无搜索后端 |
...
| MCP Connector | ✅ | ❌ | ⚠️ | header 接受但无法实际连接 |
| Memory Tool | ✅ | ❌ | ❌ | **完全不支持** |
| Bash Tool | ✅ | ✅ | ✅ | 无 | [test_17](test_17_bash_tool.py) |
| Text Editor Tool | ✅ | ✅ | ✅ | name 差异 | [test_18](test_18_text_editor_tool.py) |
...
| **Bedrock** | `web_search_20250305` 是 Bedrock 认可的 tool type（不报 "does not match expected tags"），`web-search-2025-03-05` beta header 也被接受，但实际调用返回 "The provided request is not valid"。`web_search_20260209` 不被识别。Bedrock 无搜索后端执行能力 |
...
| **Bedrock** | `mcp-client-2025-11-20` beta header 被接受，`mcp_servers` 字段可被解析和验证，但实际连接 MCP 服务器时返回 "The provided request is not valid" |
| **Gap** | Bedrock 接受 MCP 相关参数但无法实际连接外部 MCP 服务器 |
